// RITE VIII-3 wave (b) — THE UPSCALER AT SPEED: the GPU compute port of the
// VIII-3 CPU reference upscaler (src/upscaler.rs). A per-TARGET-pixel MLP that
// predicts a RESIDUAL over the bilinearly-upsampled low-resolution radiance,
// worked in the albedo-demodulated log-radiance space VIII-1 established.
// Same hash-pinned weights, LOADED (uploaded) not re-derived; same feature
// engineering, same bilinear resample, and the SAME fixed accumulation order
// as the CPU forward pass, so the two agree to an fp32-rounding tolerance
// (parity ordeal, viii3b_ordeals) — the exact house pattern the denoiser port
// (denoiser.wgsl) set.
//
// THE BAN (this file carries the marker below so the VIII-0 grep-gate scope
// and the VIII-3 shader scan pick it up): every value here is a function of
// THIS frame's buffers alone — the low-res radiance of THIS frame and THIS
// frame's target-resolution albedo/normal/depth. No accumulation across
// frames, no read of any earlier output, no parameter carrying a frame index
// or an earlier buffer. Space is reconstructed WITHIN one honestly traced
// frame; one dispatch is the complete, deterministic answer for this frame.
//
// BAN-SCOPED

// Fixed ceilings — the shipped net (4 hidden layers, width 64; input 21,
...
// activation scratch (input 21 ≤ 64, all hidden 64 ≤ 64). 64 mirrors the
// denoiser port's measured-best scratch width on this M1/naga/Metal combo.
...
// LOW-resolution radiance, one vec4 per LOW pixel: (radiance.rgb, _).
@group(0) @binding(2) var<storage, read> low_radiance: array<vec4<f32>>;
// TARGET-resolution auxiliary geometry, one vec4 per TARGET pixel:
//   hi_albedo[i] = (albedo.rgb, _)
//   hi_normal[i] = (normal.xyz, _)
//   hi_depth[i]  = depth (scalar, but stored as vec4 for alignment: (d,_,_,_))
...
// out_img[i] = (upscaled.rgb, 0), one per TARGET pixel.
@group(0) @binding(6) var<storage, read_write> out_img: array<vec4<f32>>;

// Mirrors src/upscaler.rs::demod_divisor EXACTLY.
...
// log-demod (src/upscaler.rs::log_demod): ln(1 + max(radiance/divisor, 0)).
...
// undo_log_demod (src/upscaler.rs::undo_log_demod): (expm1, clamp≥0) * divisor.
...
fn upscale(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // bilinear_taps (src/upscaler.rs): 4 clamped integer taps + (dx, dy).
  ...
  let dx = fx - x0f;
  let dy = fy - y0f;
  // clampi(v, hi) = min(max(v,0) as u32, hi-1)
  ...
  let t00 = y0i * low_w + x0i;
  let t10 = y0i * low_w + x1i;
  let t01 = y1i * low_w + x0i;
  let t11 = y1i * low_w + x1i;

  // bilinear base (src/upscaler.rs::bilinear_upsample, same mix order).
  let c00 = low_radiance[t00].xyz;
  let c10 = low_radiance[t10].xyz;
  let c01 = low_radiance[t01].xyz;
  let c11 = low_radiance[t11].xyz;
  ...
  // pixel_features (src/upscaler.rs): 4 taps log-demod (12) + dx,dy (2) +
  // hi_albedo (3) + hi_normal (3) + log depth (1) = 21, in THIS order.
  ...
  // out_dl = base_dl + residual; out = undo_log_demod(out_dl, divisor).
  let out_dl = vec3<f32>(base_dl.x + act[0], base_dl.y + act[1], base_dl.z + act[2]);
