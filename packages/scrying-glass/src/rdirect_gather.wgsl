// NEURAL-LIVE N0.b — FEATURE GATHER (the net's 23-feature input tensor built
// on the GPU from the live frame's existing buffers). One invocation per
// TARGET pixel. Bit-for-bit the CPU `rdirect::pixel_features` / `bilinear_taps`
// (parity-gated in tests/rdirect_gather_ordeals.rs): 2×2 low-res demod-log
// radiance taps (12) + subpixel offset (2) + hi-res albedo (3) + normal (3) +
// log depth (1) + screen-space motion (2) = 23.
//
// Inputs are the SAME buffers the live path already holds:
//   accum : trace-resolution accumulation cells (rgb = radiance sum, w = sample
//           count), resolved here as sum/max(count,1) — the integrator's
//           `accum` / `resolve()`.
//   aov   : native-resolution primary-hit AOVs, 2 vec4 cells/pixel
//           (integrator.wgsl "VIII-0 AOV EXPORT"):
//             aov[2*i+0] = (albedo.rgb, depth)
//             aov[2*i+1] = (normal.xyz,  hit)
//
// CURRENT-FRAME ONLY — no prior-frame read, no cross-frame state.
// BAN-SCOPED
...
fn gather(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // Native-res primary-hit AOVs for this target pixel.
  let a = aov[2u * i + 0u];
  let b = aov[2u * i + 1u];
  let albedo = a.xyz;
  let depth = a.w;
  ...
  k = k + 1u;
  // Screen-space motion — zeroed this wave (matches the exported fixed-pose
  // reference; live motion vectors are a later wave).
  feats[base + k + 0u] = 0.0;
  feats[base + k + 1u] = 0.0;
}
