...
/// The body is composed at SAMA's idle pose (via [`vessel::Body`]) and given a
/// live [`Locomotion`] machine for V1; each vertex is coloured by its region
/// palette, a triangle's albedo the mean of its three vertices' linear colours.
///
...
/// is nudged by eye. `body.preset` names the preset; an unknown preset is an
/// authoring error (surfaced, never silently invented).
