//! R-DIRECT v8d — K-AVERAGED NOISE2NOISE TARGETS (2026-07-21). SINGLE
//! VARIABLE off v8c (`rdirect_train_v8c.rs`, the doctrine round): v8c's
//! verdict (`scratch/v8c-train.log`) held doctrine (TIER1 init: no
//! detonation, sparkle stayed ~90 flat, mirror sparkle 0 throughout) but
//! val_resid sat FLAT at 0.0432 for all 200 epochs, never approaching the
//! 0.035 bar — diagnosis: TIER2's noise2noise target was a SINGLE
//! independent evidence draw (draw B, 1 spp) per pixel per step. A 1-ray
//! target's variance is the full per-pixel noise variance of the
//! integrator — far too high for the tiny per-batch gradient (subsample
//! 5000px/pose, batch 64) to average down enough to move the net out of
//! the TIER1 init basin; the noise2noise unbiasedness argument
//! (E[draw_B]=truth) holds in expectation but the GRADIENT SNR per step
//! was too weak.
//!
//! THE FIX (this file's only change from v8c): instead of ONE draw-B
//! sample, trace `GAIA_V8D_K` (default 8) INDEPENDENT native-res evidence
//! draws per pose per step, ONCE, at pose-sequence render time (not
//! per-epoch — same as v8c's single draw B) — target = their MEAN. Still
//! unbiased (E[mean of K iid draws] = truth, same as K=1) but variance is
//! now truth_variance/K — an 8x variance reduction at K=8, which should
//! translate to a stronger, less noisy gradient signal per step without
//! touching the estimator itself, the loss shape (still plain MSE, no
//! teacher, no cap/gate), or any other v8c mechanism. Draw indices are
//! summed/averaged as they render (running sum, not K retained buffers)
//! per the mandate's memory note — only draw_0's own low-res evidence
//! taps (unchanged from v8c's "draw A", byte-identical seed family
//! 0x7abc) are kept for the net's INPUT features; the K target draws
//! (seed family 0xB222, offset per draw index) are summed into a running
//! `radiance_sum` and only the MEAN survives into `Step.target_dl` — no
//! per-draw buffers retained, RAM cost identical to v8c's single-draw
//! case plus one transient (e,d) pair per draw.
//!
//! EVERYTHING ELSE IDENTICAL TO v8c: TIER1 estimator-init
//! (`evidence_mean_init_split`), EMA-sourced history chain, mirror pose
//! spp-4 evidence (kept unconditionally, A3-innocent), moving-camera pan
//! history (kept, A1-innocent), gamma=1.5 evidence clamp, highlight-
//! targeted sampling DROPPED (A2 convicted, same as v8c), corner-crawl lr
//! 1e-4, monitor every epoch, cross-run bar-normalized score floor
//! (tag-scoped, default tag "v8d" — can never read/clobber v8 or v8c's
//! checkpoints/floors), FRESH init only, detach for the real 200-epoch run.
//!
//! Run: cargo run --release -j2 --example rdirect_train_v8d
//!   GAIA_V7_SKY_HISTORY=reject GAIA_V8D_K=8   (K defaults to 8; mandate default)
//!
//! ── v8c's own header follows verbatim (historical context, all of the
//! below still describes what THIS file inherits unchanged except the one
//! diff above) ──
//!
...
struct Step {
    ...
    target_dl: Vec<[f32; OUTPUT_CHANNELS]>,  // v8d: MEAN of K independent draws' radiance, demod-log — the noise2noise LABEL
...
fn render_pose_seq(
    ...
    for step in 0..k {
        ...
        // v8d — K-AVERAGED TARGET: K INDEPENDENT sparse traces, native
        // resolution, SAME spp as draw A, DIFFERENT seed family per draw
        // index — never fed to the net, purely the noise2noise training
        // LABEL. Unfiltered per-draw (no blur, no ref_frames accumulation:
        // filtering would bias the label away from an unbiased per-pixel
        // radiance estimate) but AVERAGED across K draws — still unbiased
        // (E[mean of K iid draws] = truth) with variance/K vs v8c's K=1.
        // Running sum only — no per-draw buffers retained (RAM note in the
        // mandate: cache the MEAN, not the K draws).
...
fn main() {
    let run_tag = std::env::var("GAIA_V8_TAG").unwrap_or_else(|_| "v8d".to_string());
    ...
    let k_draws = env_u32("GAIA_V8D_K", 8); // v8d: # independent evidence draws averaged into the noise2noise target
    eprintln!("[{run_tag}] GAIA_V8D_K={k_draws} — noise2noise target = mean of {k_draws} independent draws (variance/{k_draws} vs v8c's K=1)");
    ...
    eprintln!(
        "[{run_tag}] rendered {}+2 MOVING/STILL pose sequences (K={k} steps, {tw}x{th}, teacher {ref_frames} [VALIDATOR ONLY], pan_step={pan_step}, mirror_spp={mirror_spp}, v8d K-averaged target draws={k_draws}, seed base=0x{DRAW_B_SEED_BASE:x}) in {:.1}s",
    ...
    assert_eq!(mlp.layer_dims()[0].0 as usize, HIST_FEATURES_SPLIT, "v8d net must be 39-input");
    ...
    eprintln!(
        "[{run_tag}] arch {:?} in={HIST_FEATURES_SPLIT} macs/px={} — {epochs} epochs (wall<={wall_budget}s), subsample {subsample}px/pose (uniform, NO highlight bias — A2 convicted), lr0={lr0} (mild harmonic decay), ema={ema_decay}, TIER2 loss = plain MSE vs K={k_draws}-averaged draw radiance (v8d noise2noise, variance/{k_draws}, teacher NEVER in the loss)",
        config, mlp.macs(),
    );
    std::io::stderr().flush().ok();

    // Cross-run bar-normalized score floor: beat the EXISTING checkpoint FOR
    // THIS TAG (v8d's own floor file — tag-scoped, can never read/clobber
    ...
    let prov = serde_json::json!({
        ...
        "weights_sha256": wsha,
        "supersedes": "rdirect-weights-v8c.bin",
        "doctrine_concordance": "TIER1(estimator-init)+TIER2(K-averaged noise2noise, K={k_draws}, teacher=validator-only)+TIER3(structure: moving-camera history + mirror pose + EMA history-source + gamma=1.5 clamp, kept as validator/structural, NOT the training signal) — v8d single-variable diff off v8c (variance/{k_draws} target)",
        "architecture": {
            "kind": "N7 MOVING-HISTORY + SPECULAR-EVIDENCE + TIER1-ESTIMATOR-INIT + TIER2-K-AVERAGED-NOISE2NOISE — recurrent direct-render MLP, split radiance input (E+D), 39-in/3-out, evidence-clamped act",
        ...
        "training": {
            ...
            "loss": format!("v8d TIER2 K-averaged noise2noise: plain MSE(net(draw_0 features), mean of K={k_draws} independent draw radiances), backprop through the v7e evidence clamp — teacher NEVER in the loss, validator only. variance/{k_draws} vs v8c's K=1"),
            "k_draws": k_draws,
