...
gather (N0.b, pooled zero-copy MTLBuffer) → MPSGraph batched-GEMM forward [source: this doc's own measurement logs, cited in full at doc head]
...
- world: `worlds/naruko` (default) · M1 / macOS 26 [source: this doc's own measurement logs, cited in full at doc head]
...
| net      | 27.12  | 28.57 | MPSGraph 23→5×64 ReLU→3 forward over 614 400 px    | [source: this doc's own measurement logs, cited in full at doc head]
...
gap is **CPU-side MPSGraph per-frame cost**: `encodeToCommandBuffer` building [source: this doc's own measurement logs, cited in full at doc head]
...
wall — the wall was never the GPU.** MPSGraph's per-encode CPU overhead + the [source: this doc's own measurement logs, cited in full at doc head]
...
The GPU forward fits (~4.5 ms). The wall is MPSGraph's CPU-side per-frame [source: this doc's own measurement logs, cited in full at doc head]
...
- **Kill the per-frame MPSGraph encode** — encode the net onto the SAME command [source: this doc's own measurement logs, cited in full at doc head]
  buffer as trace+demod (one submit, no separate MPS command buffer + wait), or
  move off MPSGraph to a hand-written MTLComputeCommandEncoder / MTL4 tensor [source: this doc's own measurement logs, cited in full at doc head]
  path whose per-call CPU encode is a fraction of MPSGraph's. [source: this doc's own measurement logs, cited in full at doc head]
...
- **Pixel words:** coherent naruko scene — brown crates, translucent mirror
  ...
  natural, radiance bounded → the GPU demod is wired right. **Visibly CLEANER [source: this doc's own measurement logs, cited in full at doc head]
...
- **S5 chain default**: the frame's default net is the raw
  ...
  MPSGraph). `GAIA_NATIVE_NET_MPSGRAPH=1` flips back to the MPSGraph executable [source: this doc's own measurement logs, cited in full at doc head]
  (A/B).
...
| net GPU  | 42.76  | 47.81 | **6.4× the MPSGraph GPU** — separate per-layer GEMMs | [source: this doc's own measurement logs, cited in full at doc head]
...
Shift 7's raw-kernel chain was chartered to "kill the MPSGraph per-frame encode [source: this doc's own measurement logs, cited in full at doc head]
wall". It DID: chain net CPU = wall−GPU = 43.15−42.76 = **0.39 ms** (vs
MPSGraph's ~13.9 ms encode). **But the trade is catastrophic:** the raw [source: this doc's own measurement logs, cited in full at doc head]
`MPSMatrixMultiplication` chain runs each layer as a separate dispatch with
intermediate buffers and NO fusion, so its **GPU time is 42.76 ms vs MPSGraph's [source: this doc's own measurement logs, cited in full at doc head]
6.65 ms — 6.4×**. Killing a ~14 ms CPU wall cost ~36 ms of extra GPU. Net:
**chain TOTAL 53.06 ms vs MPSGraph 30.05 ms — the chain is 1.8× SLOWER.** [source: this doc's own measurement logs, cited in full at doc head]
Parity is green (4.8e-7); SPEED is worse. The dispatcher keeps chain as the
default per the S5 charter, but **the honest budget says MPSGraph wins today**, [source: this doc's own measurement logs, cited in full at doc head]
and the real target is a fused GPU forward that keeps MPSGraph's ~6.6 ms GPU [source: this doc's own measurement logs, cited in full at doc head]
while shedding its ~14 ms CPU encode (MTL4 tensor / hand-fused compute), not the
un-fused chain.

## God's res dividend — the frame is CLEANER [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NATIVE_WORKER_WINDOW=true GAIA_NATIVE_NET_PRESENT=true
  ...
  binary, world `worlds/naruko`, M1 / macOS 26. Worker window = non-activating [source: this doc's own measurement logs, cited in full at doc head]
...
release, M1/macOS 26): [source: this doc's own measurement logs, cited in full at doc head]
- **S8 — MPSGraph is the frame DEFAULT.** N0.f proved the fused MPSGraph GPU [source: this doc's own measurement logs, cited in full at doc head]
  ...
  after the flip: MPSGraph(default) vs CPU **1.9e-6**, MPSGraph vs chain [source: this doc's own measurement logs, cited in full at doc head]
  **4.8e-7** (ordeal `n0b_gather_and_shared_forward_match_cpu`).
- **S9 — the ~14 ms MPSGraph encode rides a dedicated thread.** N0.f's net wall [source: this doc's own measurement logs, cited in full at doc head]
...
**S8+S9 MPSGraph pipeline (the DEFAULT, frames=300)** — [source: this doc's own measurement logs, cited in full at doc head]
...
| **TOTAL**| **45.05** | **58.76** | ~22 fps · confirms S8: MPSGraph pipeline wins 2.2× | [source: this doc's own measurement logs, cited in full at doc head]
...
- BUT **trace regressed 6.5 → 12.7 ms (+6 ms).** The A/B ISOLATES the cause: the
  ...
  6.99 ms baseline; only the MPSGraph pipeline (encode thread busy ~14 ms/frame) [source: this doc's own measurement logs, cited in full at doc head]
...
- **Encode-thread occupancy ≈ 69%** (derived: one ~13.9 ms MPSGraph encode per [source: this doc's own measurement logs, cited in full at doc head]
...
## Proof — both eyes
- MPSGraph pipeline surface: `proof/neural-live/s9-pipeline-mpsgraph-net.png` [source: this doc's own measurement logs, cited in full at doc head]
...
- **Pixel words (both, READ):** coherent naruko scene — brown crates,
  ...
  **Parity MPSGraph-pipeline ↔ chain-pipeline: HOLDS** — same [source: this doc's own measurement logs, cited in full at doc head]
  geometry/lighting/silhouettes/demod; the only diff is the animated presence
...
- env: `GAIA_NATIVE_WORKER_WINDOW=true GAIA_NATIVE_NET_PRESENT=true
  ...
  M1 / macOS 26. Tag `[n0g]`. Ownership: encode thread `rdirect-net-encode` [source: this doc's own measurement logs, cited in full at doc head]
...
M1 / macOS 26, `worlds/naruko`, `GAIA_NATIVE_OFFSCREEN=true`. [source: this doc's own measurement logs, cited in full at doc head]
...
Mechanism: **MPSGraph's `encodeToCommandBuffer` internally `commitAndContinue`s**, [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NEURAL_LIVE=1 GAIA_NATIVE_OFFSCREEN=true GAIA_NATIVE_NET_PRESENT=true
  GAIA_NATIVE_PORT=8438`, release, world `worlds/naruko`, M1 / macOS 26. [source: this doc's own measurement logs, cited in full at doc head]
...
640×480, release, M1/macOS 26, `worlds/naruko`, `GAIA_NATIVE_OFFSCREEN=true`. [source: this doc's own measurement logs, cited in full at doc head]
...
| stage    | median | p95   | note                                                  |
|----------|--------|-------|-------------------------------------------------------|
| trace    | 5.93   | 7.54  | +0.2 ms — net GPU now contends on the shared M1 GPU    | [source: this doc's own measurement logs, cited in full at doc head]
...
1. **Single-GPU serialization tax.** The M1 has ONE GPU: deferring the CPU wait [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NEURAL_LIVE=1 GAIA_NATIVE_OFFSCREEN=true GAIA_NATIVE_NET_PRESENT=true
  GAIA_NATIVE_HUD=false`, release, world `worlds/naruko`, M1/macOS 26. Tag `[n0i]`. [source: this doc's own measurement logs, cited in full at doc head]
...
brings back the honest number. Live, offscreen 640×480, release, M1/macOS 26, [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NEURAL_LIVE=1 GAIA_NATIVE_OFFSCREEN=true GAIA_NATIVE_NET_PRESENT=true
  GAIA_NATIVE_HUD=false`, release, world `worlds/naruko`, M1/macOS 26. [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NATIVE_OFFSCREEN=true GAIA_NATIVE_SLEEP=1 [GAIA_NATIVE_SLEEP_VEL/_FRAMES]`,
  release, world `worlds/naruko`, M1/macOS 26. Toggle sleep off = omit the env. [source: this doc's own measurement logs, cited in full at doc head]
...
release, M1/macOS 26, `worlds/naruko`, `GAIA_NATIVE_OFFSCREEN=true`. [source: this doc's own measurement logs, cited in full at doc head]
...
- `n0b_gather_and_shared_forward_match_cpu` — ok (GATE A 9.5e-7, GATE B
  1.9e-6, S8 MPSGraph-vs-chain 4.8e-7; release, `GAIA_NEURAL_LIVE=1`). [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NEURAL_LIVE=1 GAIA_NATIVE_OFFSCREEN=true GAIA_NATIVE_NET_PRESENT=true
  ...
  `worlds/naruko`, M1/macOS 26. Tag `[n0m]` (reuses `[n0i]` per-frame log lines [source: this doc's own measurement logs, cited in full at doc head]
...
against the net forward on the NET queue (single M1 GPU), so its submission ate a [source: this doc's own measurement logs, cited in full at doc head]
...
against the net forward on the single M1 GPU: `gather` ballooned 0.93→9.3 med [source: this doc's own measurement logs, cited in full at doc head]
...
de-serializes trace against the net on the single M1 GPU and regresses −4 to −5 [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NEURAL_LIVE=1 GAIA_NATIVE_OFFSCREEN=true GAIA_NATIVE_NET_PRESENT=true
  ...
  `GAIA_NATIVE_ASYNC_TRACE=1` (CUT B, rejected), release, `worlds/naruko`, M1/macOS 26. [source: this doc's own measurement logs, cited in full at doc head]
- 0 scrying-glass processes at shift end (every server `kill`ed via `trap … EXIT`,
  confirmed `ps aux`).

## Adversary — S18 current
CUT A's win is a SCHEDULING win, not a compute cut: the single M1 GPU still runs [source: this doc's own measurement logs, cited in full at doc head]
...
`s20-v2.budget.json`, frames=1426, `worlds/naruko`, M1/macOS 26: [source: this doc's own measurement logs, cited in full at doc head]
...
- env: `GAIA_NEURAL_LIVE=1 GAIA_NATIVE_OFFSCREEN=true GAIA_NATIVE_HUD=false
  ...
  `worlds/naruko`, M1/macOS 26. Windowed proofs: `GAIA_NATIVE_WORKER_WINDOW=true` [source: this doc's own measurement logs, cited in full at doc head]
