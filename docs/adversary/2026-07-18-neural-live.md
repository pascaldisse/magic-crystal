...
- **S8 flipped the default to MPSGraph** (the 1.8× faster path N0.f measured); [source: this report's own N0/N4 measurements, cited in full below]
  the chain is now the opt-in lab A/B (`GAIA_NATIVE_NET_CHAIN=1`). Parity
  re-gated: MPSGraph vs CPU 1.9e-6, MPSGraph vs chain 4.8e-7. The default is no [source: this report's own N0/N4 measurements, cited in full below]
  longer the slower path — the shift-8 concordance gap #2 is CLOSED.
- **S9 pipelined the ~14 ms MPSGraph encode onto a background thread** (double- [source: this report's own N0/N4 measurements, cited in full below]
...
AOV → GPU feature gather → net forward (chain default / MPSGraph A/B) → GPU [source: this report's own N0/N4 measurements, cited in full below]
...
- **Parity — HOLDS.** Chain vs CPU 1.9e-6, chain vs MPSGraph 4.8e-7 (ordeal [source: this report's own N0/N4 measurements, cited in full below]
...
- **Budget — FAILS the 60-fps law.** Neither path hits 16.67 ms @640×480:
  chain TOTAL **53.06 ms** (~19 fps, 3.2× over), MPSGraph **30.05 ms** (~33 fps, [source: this report's own N0/N4 measurements, cited in full below]
...
- **The chosen default is the slower path.** S5 makes the raw chain the frame
  ...
  MPSGraph alternative it replaced. Default stands per the S5 charter, but it is [source: this report's own N0/N4 measurements, cited in full below]
...
| stage    | CHAIN med/p95 | MPSGraph med/p95 | [source: this report's own N0/N4 measurements, cited in full below]
|----------|---------------|------------------|
...
MPSGraph's ~13.9 ms CPU encode (chain CPU = 43.15−42.76 = 0.39 ms) but its [source: this report's own N0/N4 measurements, cited in full below]
...
Real target: a FUSED GPU forward that keeps MPSGraph's ~6.6 ms GPU and sheds its [source: this report's own N0/N4 measurements, cited in full below]
...
1. **60 fps unmet** — net stage is the wall on BOTH paths. Chain: 42.8 ms GPU
   (un-fused). MPSGraph: 6.6 ms GPU + 13.9 ms CPU encode. The win is a fused [source: this report's own N0/N4 measurements, cited in full below]
...
2. **Default = slower path.** S5 charter keeps chain default though MPSGraph is [source: this report's own N0/N4 measurements, cited in full below]
...
- **The wedge — MEASURED, cured.** S12/S12.5's dedicated-net-queue +
  ...
  Instrumented root cause (`GAIA_NATIVE_NET_TRACE`, see perf N0.h): MPSGraph's [source: this report's own N0/N4 measurements, cited in full below]
...
3. Prior gaps (default = slower path is now RESOLVED — S8 flipped MPSGraph to [source: this report's own N0/N4 measurements, cited in full below]
