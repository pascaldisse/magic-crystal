...
| ANANKE (sealed 07-18) | the constraint machinery formerly called "the solver" (tech term banned): Necessity herself — what must happen, happens; enforces love's budgets on all matter, one door (ops) into her law; even gods don't negotiate. Empedocles' Love & Strife row = her forces; Ananke = the engine that applies them |
| die WILDE JAGD (sealed 07-18) | the ordeal suite + adversary rides: ~400 self-contained trials, each builds a small world and checks ONE law holds; rides at every merge (the liminal threshold); HOLDS = ridden past, MUST-FIX = swept away. Folklore law made literal: the taken JOIN the hunt — adversary constructions become permanent ordeals (adv_margin.rs, adv_chain.rs); the horde grows only by its prey. The hounds (profiler, profile-seam) run ahead and find the scent |
| die ZAUBERPOLIZEI (sealed 07-18) | the friendly face of the law — "dein Freund und Helfer": tutorial (teaches world-law instead of letting you die of it), lint/check patrols, and the world-compiler's voice (a lawless spoken world gets a POLICE VISIT explaining the broken law, not a cold error). Their lore-quest = the search for the Universal Source Code: schema/docs/canon are their case files. The Jagd punishes; the Zauberpolizei teaches, warns, investigates. Their diagnostic art = FIRASA (Sufi): reading the inner state from outer signs — lint infers the fault without running the code |
| KASHF — die Entschleierung (Sufi pillar, 07-18) | mind-reading of the LIVING world: VisionFlow node-debugging — toggle the view, the veil lifts, the world's thoughts lie open DIRECTLY (values pulsing live through node veins), no inference, unveiling. Huginn & Muninn = its two couriers: Thought (live state) and Memory (traces/logs/history). Momus' window finally built: the gods laughed at his demand for a window in the breast to see thoughts — VisionFlow installs it |
| der TRAUMDEUTER (07-18) | breakpoint debugging: the world falls into Dornröschenschlaf — every callstack frozen mid-gesture — and one walker reads every thought in the sleeping castle; on waking, the world never knows it slept (its subjective time never stopped). Already ritual: ■ stop, player.frozen, '+'-snapshot |
